...
import static org.apache.flink.statefun.playground.java.shoppingcartosp.Messages.*;
import static org.apache.flink.statefun.playground.java.shoppingcartosp.Messages.ItemAvailability.*;
...
final class StockFn implements StatefulFunction {
  ...
  static final TypeName TYPE = TypeName.typeNameFromString("statefun.ndb/stock");
  static final ValueSpec<Integer> STOCK = ValueSpec.named("stock").withIntType();

  @Override
  public CompletableFuture<Void> apply(Context context, Message message) {
    AddressScopedStorage storage = context.storage();
    final int quantity = storage.get(STOCK).orElse(300);
    if (message.is(RESTOCK_ITEM_TYPE)) {
      ...
      if (quantity >= requestQuantity) {
        ...
        itemAvailability =
            new ItemAvailability(Status.INSTOCK, requestQuantity, request.getTimestamp());
      } else {
        itemAvailability =
            new ItemAvailability(Status.OUTOFSTOCK, requestQuantity, request.getTimestamp());
      }
