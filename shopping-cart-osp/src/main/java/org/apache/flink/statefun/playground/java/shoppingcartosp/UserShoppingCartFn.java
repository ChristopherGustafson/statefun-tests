...
final class UserShoppingCartFn implements StatefulFunction {
  ...
  static final TypeName TYPE = TypeName.typeNameFromString("statefun.ndb/user-shopping-cart");
  static final ValueSpec<Basket> BASKET = ValueSpec.named("basket").withCustomType(Basket.TYPE);

  @Override
  public CompletableFuture<Void> apply(Context context, Message message) {
    LOG.info("ShoppinCartFn invocated with message: " + message.toString());
    if (message.is(Messages.ADD_TO_CART)) {
      final AddToCart addToCart = message.as(Messages.ADD_TO_CART);
      LOG.info("{}", addToCart);
      LOG.info("Scope: {}", context.self());
      LOG.info("Caller: {}", context.caller());

      final Messages.RequestItem requestItem =
          new Messages.RequestItem(addToCart.getQuantity(), addToCart.getPublishTimestamp());
    ...
    if (message.is(Messages.ITEM_AVAILABILITY_TYPE)) {
      ...
      if (Messages.ItemAvailability.Status.INSTOCK.equals(availability.getStatus())) {
        ...
        // Send to egress for benchmark metrics calculation
        final AddToCart confirm =
            AddToCart.newBuilder()
                .setUserId(context.self().id())
                .setItemId(caller.get().id())
                .setQuantity(availability.getQuantity())
                .setPublishTimestamp(availability.getTimestamp())
                .build();
        final EgressMessage egressMessage =
            KafkaEgressMessage.forEgress(Identifiers.ADD_CONFIRM_EGRESS)
                .withTopic(Identifiers.ADD_CONFIRM_TOPICS)
                .withUtf8Key(context.self().id())
                .withValue(Messages.ADD_CONFIRM_TYPE, confirm)
                .build();
              ...
              basket -> {
                for (Map.Entry<String, Integer> entry : basket.getEntries()) {
                  Messages.RestockItem restockItem =
                      new Messages.RestockItem(entry.getKey(), entry.getValue(), "empty-timestamp");
    ...
    if (message.is(Messages.CHECKOUT_TYPE)) {
      ...
      itemsOption.ifPresent(
          items -> {
            ...
            final Messages.Receipt receipt =
                new Messages.Receipt(context.self().id(), items, checkout.getTimestamp());
            final EgressMessage egressMessage =
